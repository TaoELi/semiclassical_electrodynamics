...
class TwoSites:
    def __init__(
        ...
        self.count = 0
        self.new_x_range = 0
        self.gauss_freq = gauss_freq
        self.frequency = self.gauss_freq / (2 * np.pi)
        self.frequencyWidth = 0.1 / self.rescaling
        self.size = size
    ...
    def propogate_Eh(self, int_EP_L, int_EP_R):
    ...
    def _step_function2(self):
        def __step_func2__(sim):
            if self.count % self.print_every == 0:
                self.t_lst.append(self.t) #time. 
                self.EM_energy.append(1/2*self.energy) #EM energy
            #Integration box. This matches the cavity size.
            Integration_box=mp.Volume(size=mp.Vector3(0.5 * self.rescaling, 3/5*self.length-2*self.pmlThickness, 0), center=mp.Vector3(0,0.0,0.0))
            #Electric and magnetic field components in order to calculate the integral over the integration box
            ...
            #Integration procedure for each time step using metadata approach -> https://meep.readthedocs.io/en/latest/Python_User_Interface/#output-functions
            (x,y,z,w) = sim.get_array_metadata(vol=Integration_box)
            electric_energy_density = np.real(eps*(np.conj(Ex)*Ex + np.conj(Ey)*Ey + np.conj(Ez)*Ez))    # array
            magnetic_energy_density = np.real(1/perm*(np.conj(Hx)*Hx + np.conj(Hy)*Hy + np.conj(Hz)*Hz)) # array
            energy_density = (electric_energy_density+magnetic_energy_density)           # array
            self.energy = np.sum(w*energy_density)                                       # scalar                        
            #Update time
            self.t += self.dt
            self.count += 1
        return __step_func2__
    ...
    def integrate(
        ...
        self.sim.run(self._step_function(),self._step_function2(),until=tmax)
    ...
    def output_EM_energy(self, traj_filename="EM_energy.txt"):
        # output data finnally
        with open(traj_filename, "w") as handle:
            for t, energy in zip(self.t_lst, self.EM_energy):
                handle.write("%.4E %.8E\n" % (t, np.abs(energy)))
